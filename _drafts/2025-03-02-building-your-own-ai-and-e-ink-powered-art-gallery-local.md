...
We build a simple **E-ink** picture **ESP32** frame that displays a new
piece of artwork every morning, generated by **local AI diffusion model**.
The content changes daily, so each day brings a completely new and random image to enjoy.
Everything is run locally on our own network, ensuring that the process is
private and keeps everything off the cloud.
It's a straightforward way to have dynamic, AI-generated art on your walls
without compromising on privacy.
This setup naturally extends our Home Assistant smart home setup, which hosts the image server and keeps track of the ESP32s.

Want to build your own? Here is the recipe.
If you want wireless you would want to use ESP32 chip, which requires some soldering.
If you don't want to solder you can build it with a Raspberry Pi Zero.
